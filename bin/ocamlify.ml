...
type var_type = VarString | VarStringList

let version = "%%VERSION%%"

let to_ocaml_string str =
  let ocaml_str = Printf.sprintf "%S" str in
  if
    String.length ocaml_str >= 2
    && ocaml_str.[0] = '"'
    && ocaml_str.[String.length ocaml_str - 1] = '"'
  then String.sub ocaml_str 1 (String.length ocaml_str - 2)
  else str

(** Create output file containing variable *)
let ocamlify fd var_lst =
  let output_var fd vartype varname file =
    let fpf str = Printf.fprintf fd str in
    let iter_line f =
    ...
    | VarString ->
        fpf "  \"\\\n";
        iter_line (fun str ->
            let ocaml_str = to_ocaml_string str in
...
let depends fd var_lst fn =
  List.iter
    (fun (_, _, fnd) ->
      (match fn with None -> () | Some fn -> output_string fd (fn ^ ": "));
      output_string fd (fnd ^ "\n"))
    var_lst

(** Read configuration file *)
let parse_file =
  let lexer = make_lexer [ "VarString"; "VarStringList" ] in
  let rec to_list acc str =
    try to_list (Stream.next str :: acc) str
  ...
  let rebase_fn fn_base fn =
    (* TODO: FilePath.Unix ! *)
    if Filename.is_relative fn then Filename.dirname fn_base ^ "/" ^ fn else fn
  in
  let rec analyze fn acc lst =
    match lst with
    | Kwd "VarString" :: Ident var :: String fni :: tl ->
        analyze fn ((VarString, var, rebase_fn fn fni) :: acc) tl
    | Kwd "VarStringList" :: Ident var :: String fni :: tl ->
        analyze fn ((VarStringList, var, rebase_fn fn fni) :: acc) tl
    | Kwd s :: _ | Ident s :: _ | String s :: _ -> error fn s
    | Int i :: _ -> error fn (string_of_int i)
    | Float f :: _ -> error fn (string_of_float f)
    | Char c :: _ -> error fn (String.make 1 c)
    | [] -> acc
  in
  fun fn acc ->
    let fd = open_in fn in
    let toks = to_list [] (lexer (Stream.of_channel fd)) in
    let nacc = analyze fn acc toks in
    close_in fd;
    nacc

type action = Depends | OCamlify
  ...
  let () =
    Arg.parse
     ...
     ^ " by Sylvain Le Gall \n\n\
        ...
        Options: \n")
  in
  let () = all_var := List.rev !all_var in

  let act fd var_lst =
    match !action with
    | Depends -> depends fd var_lst !target
    | OCamlify -> ocamlify fd var_lst
  in
  match !output with
  | Some fl -> (
      let fd = open_out fl in
      try
        act fd !all_var;
        close_out fd
      with e ->
